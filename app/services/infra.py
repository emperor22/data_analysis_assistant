...
from app.exceptions import BlacklistedDatasetException, RetryableRateLimitException, TerminalRateLimitException
from app.crud import BlacklistedDatasetsTableOperation, PromptTableOperation, TaskRunTableOperation
...
def check_if_task_is_valid(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        if not request_id:
            raise HTTPException(status_code=500, detail='the check_if_task_is_valid decorator expects request id argument')
        ...
        if await is_task_invalid_or_still_processing(request_id, user_id, prompt_table_ops):

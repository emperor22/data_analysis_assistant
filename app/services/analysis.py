...
class DataAnalysisProcessor:
    def __init__(self, data_tasks: DataTasks, run_info: RunInfo, task_run_table_ops: TaskRunTableOperation, 
                 run_type: str):
    ...
    def process_all_tasks(self):       
        if self.run_type in (TaskProcessingRunType.first_run_after_request.value, TaskProcessingRunType.modified_tasks_execution_with_new_dataset.value):
            ...
            save_dataset_req_id(request_id=self.request_id, dataframe=self.df, run_type=self.run_type)
        ...
        if self.send_result_to_email:
            ...
            generate_report_and_archive(request_id=self.request_id, run_type=self.run_type, result_dct=result_dct, run_name=self.run_name)
    ...
    def _process_common_tasks(self):
        ...
        for task in common_tasks:
            ...
            if error_steps:
            ...
            elif len(tmp) > Config.MAX_DATASET_ROW_RESULT or len(tmp.columns) > Config.MAX_DATASET_COLS_RESULT:
                task_modified['status'] = 'task failed because output is too big to export'
                task_modified['result'] = {}
            else:
                ...
                if not (len(tmp) > Config.DATASET_ROW_THRESHOLD_BEFORE_EXPORT or len(tmp.columns) > Config.DATASET_COLUMNS_THRESHOLD_BEFORE_EXPORT):
                    ...
                    task_modified['status'] = 'output of this analysis is too big and exported to excel'
                    task_modified['result'] = {}
...
def get_task_category(run_type):
    task_category_dct = {TaskProcessingRunType.first_run_after_request.value: 'original_tasks', 

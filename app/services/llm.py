...
class DatasetProcessorForPtOnePrompt:
    ...
    def _get_column_unique_values(self) -> dict:
        ...
        for col in self.dataframe.columns:
            ...
            unique_val_dct[col]["unique_values_count_ratio_to_row_count"] = (
                self.dataframe[col].nunique() / self.dataset_row_count
            )
            unique_val_dct[col]["top_5_values"] = (
                self.dataframe[col].value_counts().index[:5].tolist()
            )
...
async def check_if_api_key_valid_cerebras(key):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            Config.LLM_ENDPOINT_CEREBRAS, headers=headers, json=payload
        )
...
def cleanup_agg_col_names(resp_pt_2, resp_pt_1):
    ...
    mrg_lst = [
        *resp_pt_1["columns"],
        *resp_pt_1["common_column_combination"],
        *resp_pt_1["common_column_cleaning_or_transformation"],
    ]

...
def get_prompt_result_task(
    ...
    with engine.connect() as conn:
        ...
        check_if_dataset_is_not_blacklisted(blacklist_table_ops=blacklist_table_ops, 
                                            dataset_id=dataset_id)

        # getting result for prompt part 1                
        resp_pt_1 = get_llm_response(prompt=prompt_pt_1, 
                                     model=model, 
                                     provider=provider, 
                                     api_key=api_key, 
                                     user_id=user_id, 
        ...
        write_debug_prompt_and_response_if_requested(enabled=debug_prompt_and_res,
                                                     prompt=prompt_pt_1, 
                                                     response=resp_pt_1, 
                                                     part=1, 
                                                     request_id=request_id)
        ...
        resp_pt_1 = validate_initial_prompt_resp_part_one(response=resp_pt_1, 
        ...
        prompt_pt_2 = build_part_two_prompt(part_one_result=resp_pt_1, 
                                            task_count=task_count)

        resp_pt_2 = get_llm_response(prompt=prompt_pt_2, 
                                     model=model, 
                                     provider=provider, 
                                     api_key=api_key, 
                                     user_id=user_id, 
        ...
        resp_pt_2 = validate_initial_prompt_resp_part_two(response=resp_pt_2, 
                                              request_id=request_id, 
                                              user_id=user_id, 
                                              dataset_id=dataset_id, 
                                              blacklist_table_ops=blacklist_table_ops, 
        ...
        log_slow_prompt_tasks(process_time_ms=process_time_ms, request_id=request_id, user_id=user_id, task='initial analysis')
...
def get_additional_analyses_prompt_result_task(
    ...
    with engine.connect() as conn:
        ...
        prompt = build_additional_analysis_prompt(new_tasks_prompt=new_tasks_prompt, 
                                             request_id=request_id, 
                                             user_id=user_id, 
                                             prompt_table_ops=prompt_table_ops, 
                                             mock_file=mock_addt_request_resp_file)
        
        resp = get_llm_response(prompt=prompt, 
                                model=model, 
                                provider=provider, 
                                api_key=api_key, 
                                user_id=user_id, 
        ...
        resp = validate_additional_analyses_prompt_res(resp=resp, 
                                          user_id=user_id, 
                                          request_id=request_id, 
                                          prompt_table_ops=prompt_table_ops)
        ...
        data_tasks = build_additional_analyses_data_task(resp=resp, 
                                                         request_id=request_id)
        ...
        log_slow_prompt_tasks(process_time_ms=process_time_ms, request_id=request_id, user_id=user_id, task='additional analyses')

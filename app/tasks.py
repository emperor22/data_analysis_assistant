...
app.conf.task_routes = {
    "tasks.get_prompt_result_task": {"queue": "io_tasks_queue"},
...
@app.task(
    bind=True,
    base=DatabaseTask,
    name="tasks.get_prompt_result_task",
    acks_late=True,
    time_limit=200,
...
def get_prompt_result_task(
    ...
    with engine.connect() as conn:
        ...
        try:
            resp_pt_1 = DatasetAnalysisModelPartOne.model_validate(
                resp_pt_1,
                context={
        ...
        prompt_2_context = {
            "context_json": resp_pt_1,
            "task_count": task_count,
            "current_time": datetime.now().strftime("%H:%M:%S"),
        }
        prompt_pt_2 = insert_prompt_context(
            prompt_file=Config.PT2_PROMPT_TEMPLATE, context=prompt_2_context
        )
        ...
        try:
            resp_pt_2 = DatasetAnalysisModelPartTwo.model_validate(
                resp_pt_2,
                context={
                    "run_type": TaskProcessingRunType.first_run_after_request.value,
                    "request_id": request_id,
        ...
        blacklist_table_ops.reset_failed_attempt_count(
            dataset_id
        )  # reset if prompt results are valid
        ...
        if process_time_ms > Config.THRES_SLOW_INITIAL_REQUEST_PROCESS_TIME_MS:
            logger.warning(
                f"slow initial request processing time ({process_time_ms} ms): request_id {request_id}, user_id {user_id}"
            )
...
@app.task(
    bind=True,
    base=DatabaseTask,
    name="tasks.get_additional_analyses_prompt_result",
    acks_late=True,
    time_limit=200,
...
@app.task(
    bind=True,
    base=DatabaseTask,
    name="tasks.data_processing_task",
    acks_late=True,
    ignore_result=True,
    time_limit=20,
    max_retries=3,
...
def data_processing_task(self, data_tasks_dict, run_info, run_type):
    ...
    with engine.connect() as conn:
        ...
        prompt_table_ops.change_request_status_sync(
            request_id=request_id, status=starting_status_dct[run_type]
        )
        ...
        prompt_table_ops.change_request_status_sync(
            request_id=request_id, status=finished_status_dct[run_type]
        )

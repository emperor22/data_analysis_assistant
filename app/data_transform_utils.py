...
def clean_dataset(df):
    ...
    for col in df.columns:
        ...
        temp_series_nonull_idx = (
            df[col].dropna().index
        )  # for later when getting conversion success rate only for non-null rows
        ...
        if df[col].dtype == "object":
            ...
            if any(s in col for s in date_col_contains):
                ...
                temp_series_dt_nonull = temp_series_dt[
                    temp_series_dt.index.isin(temp_series_nonull_idx)
                ]
                datetime_success_rate = 1 - (
                    temp_series_dt_nonull.isna().sum() / len(temp_series_dt_nonull)
                )

                if (
                    datetime_success_rate
                    > Config.DATATYPE_CONV_SUCCESS_RATE_CLEAN_DATASET
                ):
                    df[col] = temp_series_dt
                    continue

            # check if numeric
            temp_series_numeric = pd.to_numeric(
                df[col].astype(str).str.replace(r"[^a-zA-Z0-9\s.]", "", regex=True),
                errors="coerce",
            )
            temp_series_numeric_nonull = temp_series_numeric[
                temp_series_numeric.index.isin(temp_series_nonull_idx)
            ]

            numeric_success_rate = 1 - (
                temp_series_numeric_nonull.isna().sum()
                / len(temp_series_numeric_nonull)
            )

            if (
                numeric_success_rate > Config.DATATYPE_CONV_SUCCESS_RATE_CLEAN_DATASET
            ):  # more than x % of col values are successfully converted to numeric, so keep it
                ...
                df[col] = (
                    ...
                    .str.strip()
                    .str.lower()
                    .replace(null_string_replace, np.nan)
...
def validate_columns(cols_to_check: str | list, column_list):
    ...
    if len(missing_cols) > 0:
        raise Exception(
            f"some columns mentioned in task do not exist. columns ({', '.join(missing_cols)})"
        )
...
def groupby_func(
    ...
    if isinstance(calculation, list):
        if len(calculation) > 1:
            ...
            df.columns = [
                f"{i[0]}_{i[1]}" for i in df.columns
            ]  # merging the two level of column names into one
    ...
    df = df.groupby(columns_to_group_by, as_index=False)[columns_to_aggregate].apply(
        calculation
    )
...
def filter_func(
    df: pd.DataFrame, column_name: str | list, operator: str, values: list | str
):
    ...
    if (
        isinstance(flt_value, str)
        and not is_numeric(flt_value)
        and operator in ("==", "!=")
    ):
        operator = "in"
...
def get_top_or_bottom_n_entries_func(
    df: pd.DataFrame,
    sort_by_column_name: str | list,
    order: str,
    ...
    sort_by_column_name = (
        sort_by_column_name[0]
        if isinstance(sort_by_column_name, list)
        else sort_by_column_name
    )

    asc_dct = {"bottom": True, "top": False}
    ascending = asc_dct[order]
    return df.sort_values(sort_by_column_name, ascending=ascending)[
        return_columns
    ].iloc[:number_of_entries]
...
def get_column_statistics_func(
    df: pd.DataFrame, column_name: str | list, calculation: str
):
    validate_columns(column_name, df.columns.tolist())
    df = df.copy()
    column_name = column_name[0] if isinstance(column_name, list) else column_name
...
@logger.catch(reraise=True)
def resample_data_func(
    df: pd.DataFrame,
    columns_to_aggregate: list,
    static_group_cols: list,
    frequency: str,
    date_column: str,
    calculation: str,
):
    ...
    if len(static_group_cols) > 0:
        resampled = (
            df.groupby(static_group_cols)
            .resample(frequency, on=date_column)[columns_to_aggregate]
            .apply(calculation)
            .reset_index()
        )
    else:
        resampled = (
            df.resample(frequency, on=date_column)[columns_to_aggregate]
            .apply(calculation)
            .reset_index()
        )
    return resampled
...
def apply_map_func(df, name: str, operation: dict):
    ...
    mapping = {
        str(i): j for i, j in mapping.items()
    }  # standardizing mapped values to str

    df[name] = (
        df[source_col].astype(str).map(mapping)
    )  # convert to string to get valid mapping
...
def apply_map_range_func(df, name: str, operation: dict):
    ...
    for rng in ranges:
        ...
        range_str = range_str.replace(
            "inf", f"{1e20}"
        )  # replacing inf with big number for easier parsing
...
def apply_math_op_func(df, name: str, operation: dict):
    ...
    try:
        ...
        print("invalid math_op expression")
...
def get_column_combination_func(df: pd.DataFrame, name: str, operation: str):
    ...
    try:
        ...
        print("invalid column combination expression")
...
def get_column_properties(series, is_numeric, is_datetime):
    ...
    if is_numeric:
        type_prop_dct = {
            ...
            "std": series.std(),
            "q_25th": series.quantile(0.25),
            "q_75th": series.quantile(0.75),
            "most_common_5_values": series.value_counts(normalize=True)
            .head(5)
            .to_dict(),
        ...
        type_prop_dct = {
            "datatype": "datetime",
            "most_common_5_values": series.dt.strftime("%Y-%m-%d %H:%M:%S")
            .value_counts(normalize=True)
            .head(5)
            .to_dict(),
            "date_min": min_date.strftime("%Y-%m-%d %H:%M:%S"),
            "date_max": max_date.strftime("%Y-%m-%d %H:%M:%S"),
            "range_days": pd.Timedelta(max_date - min_date).days,
            "is_categorical": is_categorical,
        }
...
def join_df_duckdb(
    df1, df2, join_keys, how: Literal["inner", "left", "right", "outer"] = "inner"
):
    conn = duckdb.connect()
    ...
    condition = " AND ".join(f"l.{left} = r.{right}" for left, right in join_keys)

...
from utils import (
    ...
    make_additional_analyses_request,
    render_request_ids,
    ...
    with analysis_req_form:
        ...
        if st.form_submit_button("Create Analysis Request"):
            if name and file and model and task_count:
                ...
                st.error(
                    "Please enter name, upload the dataset file, and choose the parameters"
                )
                time.sleep(1)
                st.rerun()
...
with additional_req_tab:
    ...
    new_analysis_text = st.text_area(
        "Type in the analyses you want here.",
        help="max 5 additional analyses; max character for each task is 60 characters, min character is 15; \

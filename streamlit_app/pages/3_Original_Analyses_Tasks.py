...
if st.sidebar.checkbox("Filter score"):
    score_threshold = st.sidebar.multiselect(
...
if (
    "modified_tasks" in st.session_state
    and request_id in st.session_state.modified_tasks
):
    col1_rst, col2_rst = st.columns([8, 2])

    with col1_rst:
        st.warning(
            "You already have imported tasks. If you want to do the import again in this page, you need to press the \
...
all_task_ids = [
    task["task_id"]
    for task in st.session_state.tasks[request_id]
...
if st.button(
    "Refresh task lists",
    help="This button is useful for when you run additional analyses and the new tasks have not appeared",
):
...
for task_idx, task in enumerate(st.session_state.tasks[request_id]):
    ...
    with col1:
        ...
        render_original_task_expander(
            request_id, task, task_idx, plots_dct, task_mode="original_tasks"
        )

    with col2:
        if id_ not in st.session_state.selected_tasks_to_modify[request_id]:
...
if st.checkbox("Check tasks in raw JSON format", key="check_original_tasks_json"):
    st.code(json.dumps(st.session_state.tasks[request_id], indent=4))

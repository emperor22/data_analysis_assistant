...
with register_tab:
    ...
    with register_form:
        ...
        st.warning('Please input a valid email as the OTPs for logins will be sent to your email.')
        ...
        if st.form_submit_button('Register'):
            ...
            res = register_user(username=username_reg, first_name=first_name, last_name=last_name, email=email)
            ...
            st.success(f'Your account with username {username} has been successfully created.')
            time.sleep(1)
            st.rerun()
